...
#define MH_TWOLEVEL (0x80) /* two-level namespace binding */
#define MH_FORCE_FLAT                                                          \
  (0x100) /* flat namespace binding (must match chained fixups) */
#define MH_HAS_TLV_DESCRIPTORS                                                 \
  (0x800000) /* thread-local variable descriptors; required for TLS */
...
static struct segment_command_64 *add_segment(struct macho *mo,
                                              const char *name) {
...
static char *tcc_search_framework_in_dir(const char *dir, const char *name) {
  static const char *const version_suffixes[] = {"", "/Versions/Current",
                                                 "/Versions/A", NULL};
  static const char *const file_suffixes[] = {"", ".tbd", NULL};
  ...
  for (i = 0; version_suffixes[i]; ++i) {
    ...
    for (j = 0; file_suffixes[j]; ++j) {
      ...
      size_t total_len = dir_len + 1 + name_len + framework_len + version_len +
                         1 + name_len + file_len + 1;
      char *path = tcc_malloc(total_len);
      snprintf(path, total_len, "%s/%s.framework%s/%s%s", dir, name,
               version_suffixes[i], name, file_suffixes[j]);
...
static char *tcc_search_framework_path_list(const char *paths,
                                            const char *name) {
...
static char *tcc_search_framework_in_sdk(const char *sdkroot,
                                         const char *name) {
  static const char *const suffixes[] = {"/System/Library/Frameworks",
                                         "/Library/Frameworks", NULL};
  int i;
...
static char *tcc_search_framework_in_library_paths(TCCState *s,
                                                   const char *name) {
  int i;
...
ST_FUNC char *tcc_get_framework_dylib_path(TCCState *s,
  ...
  if (!path)
    path = tcc_search_framework_path_list(
        getenv("DYLD_FALLBACK_FRAMEWORK_PATH"), name);
  if (!path)
...
static void tcc_macho_add_destructor(TCCState *s1) {
  ...
  mh_execute_header =
      put_elf_sym(s1->symtab, -4096, 0, ELFW(ST_INFO)(STB_GLOBAL, STT_OBJECT),
                  0, text_section->sh_num, "__mh_execute_header");
  ...
  at_exit_sym =
      put_elf_sym(s1->symtab, 0, 0, ELFW(ST_INFO)(STB_GLOBAL, STT_FUNC), 0,
  ...
  for_each_elem(s->reloc, 0, rel, ElfW_Rel) {
    ...
    ptr[1] = 0x8d;
    ptr[2] = 0x05;
    put_elf_reloca(s1->symtab, text_section, text_section->data_offset - 23,
    ...
    ptr[15] = 0x8d;
    ptr[16] = 0x15;
    put_elf_reloca(s1->symtab, text_section, text_section->data_offset - 9,
                   R_X86_64_PC32, mh_execute_header, -4);
    ptr[21] = 0xe8; // call __cxa_atexit
    put_elf_reloca(s1->symtab, text_section, text_section->data_offset - 4,
  ...
  for_each_elem(s->reloc, 0, rel, ElfW_Rel) {
    ...
    put_elf_reloc(s1->symtab, text_section, text_section->data_offset - 4,
...
static void bind_rebase_add(struct macho *mo, int bind, int sh_info,
  ...
  mo->bind_rebase = tcc_realloc(
      mo->bind_rebase, (mo->n_bind_rebase + 1) * sizeof(struct bind_rebase));
...
static void check_relocs(TCCState *s1, struct macho *mo) {
  ...
  ElfW(Sym) * sym;
  int i, j, type, gotplt_entry, sym_index, for_code, sym_is_undef;
  uint32_t *pi, *goti;
  ...
  for (i = 1; i < s1->nb_sections; i++) {
    ...
    for_each_elem(s, 0, rel, ElfW_Rel) {
      ...
      sym_is_undef =
          (sym->st_shndx == SHN_UNDEF || sym->st_shndx == SHN_FROMDLL);
      if (sym_is_undef || gotplt_entry == ALWAYS_GOTPLT_ENTRY) {
        ...
        if (!attr->dyn_index) {
          ...
          put_elf_reloc(s1->symtab, s1->got, attr->got_offset, R_JMP_SLOT,
                        sym_index);
        ...
        if (for_code && sym_is_undef) {
          ...
          if (attr->plt_offset == -1) {
            ...
            put_elf_reloc(s1->symtab, mo->stubs, attr->plt_offset,
                          R_AARCH64_ADR_GOT_PAGE, sym_index);
            write32le(jmp, // adrp x16, #sym
                      0x90000010);
            put_elf_reloc(s1->symtab, mo->stubs, attr->plt_offset + 4,
      ...
      if (type == R_DATA_PTR || type == R_JMP_SLOT) {
        /* Only add rebase/bind for sections that have data (skip NOBITS unless
         * we materialize) */
        Section *tgt = s1->sections[s->sh_info];
        if (tgt && !tgt->data && tgt->sh_type == SHT_NOBITS &&
            tgt->data_offset > 0) {
          /* Materialize NOBITS (e.g. __thread_vars) so chained fixups can be
           * written */
          section_realloc(tgt, tgt->data_offset);
          memset(tgt->data, 0, tgt->data_offset);
          tgt->sh_type = SHT_PROGBITS;
        }
        if (tgt && tgt->data) {
          ElfW_Rel rel = save_rel;
          if (strcmp(tgt->name, "__thread_vars") == 0) {
            if ((rel.r_offset % 24) != 0)
              continue; /* skip key/offset slots; only bind thunk at offset 0
                           per descriptor */
          }
          bind_rebase_add(mo, sym_is_undef ? 1 : 0, s->sh_info, &rel, NULL);
        }
      }
    }
  }
...
static void check_relocs(TCCState *s1, struct macho *mo) {
  ...
  ElfW_Rel *rel, save_rel;
  ElfW(Sym) * sym;
  int i, type, gotplt_entry, sym_index, for_code, sym_is_undef;
  ...
  jmp[1] = 0x8d;
  jmp[2] = 0x1d;
  put_elf_reloca(s1->symtab, mo->stub_helper, 3, R_X86_64_PC32,
                 mo->dyld_private, -4);
  jmp[7] = 0x41; /* pushq %r11 */
  jmp[8] = 0x53;
  jmp[9] = 0xff; /* jmpq    *dyld_stub_binder@GOT(%rip) */
  jmp[10] = 0x25;
  put_elf_reloca(s1->symtab, mo->stub_helper, 11, R_X86_64_GOTPCREL,
                 mo->dyld_stub_binder, -4);
  jmp[15] = 0x90; /* nop */
#elif defined TCC_TARGET_ARM64
  jmp = section_ptr_add(mo->stub_helper, 24);
  put_elf_reloc(s1->symtab, mo->stub_helper, 0, R_AARCH64_ADR_PREL_PG_HI21,
                mo->dyld_private);
  write32le(jmp, 0x90000011); // adrp x17, _dyld_private@page
  put_elf_reloc(s1->symtab, mo->stub_helper, 4, R_AARCH64_LDST64_ABS_LO12_NC,
                mo->dyld_private);
  write32le(jmp + 4, 0x91000231); // add x17,x17,_dyld_private@pageoff
  write32le(jmp + 8, 0xa9bf47f0); // stp x16/x17, [sp, #-16]!
  put_elf_reloc(s1->symtab, mo->stub_helper, 12, R_AARCH64_ADR_GOT_PAGE,
                mo->dyld_stub_binder);
  write32le(jmp + 12, 0x90000010); // adrp x16, dyld_stub_binder@page
  put_elf_reloc(s1->symtab, mo->stub_helper, 16, R_AARCH64_LD64_GOT_LO12_NC,
                mo->dyld_stub_binder);
    ...
    for_each_elem(s, 0, rel, ElfW_Rel) {
      ...
      if (sym_is_undef || gotplt_entry == ALWAYS_GOTPLT_ENTRY) {
        ...
        if (!attr->dyn_index) {
          ...
          if (ELFW(ST_BIND)(sym->st_info) == STB_LOCAL) {
            ...
            if (sym_is_undef && gotplt_entry == ALWAYS_GOTPLT_ENTRY) {
              mo->bind =
                  tcc_realloc(mo->bind, (mo->n_bind + 1) * sizeof(struct bind));
        ...
        if (for_code && sym_is_undef) {
          if (attr->plt_offset == -1) {
            ...
            jmp[1] = 0x25;
            put_elf_reloca(s1->symtab, mo->stubs, mo->stubs->data_offset - 4,
            ...
            put_elf_reloca(s1->symtab, mo->la_symbol_ptr,
                           mo->la_symbol_ptr->data_offset, R_DATA_PTR,
                           mo->helpsym, mo->stub_helper->data_offset - 10);
            ...
            write32le(jmp + 4, // b stubHelperHeader
                      0x14000000 + ((-(mo->stub_helper->data_offset - 8) / 4) &
                                    0x3ffffff));
            ...
            put_elf_reloca(s1->symtab, mo->la_symbol_ptr,
                           mo->la_symbol_ptr->data_offset, R_DATA_PTR,
                           mo->helpsym, mo->stub_helper->data_offset - 12);
            section_ptr_add(mo->la_symbol_ptr, PTR_SIZE);
#endif
            mo->s_lazy_bind =
                tcc_realloc(mo->s_lazy_bind,
                            (mo->n_lazy_bind + 1) * sizeof(struct s_lazy_bind));
      ...
      if (type == R_DATA_PTR || type == R_JMP_SLOT) {
        if (sym->st_shndx == SHN_UNDEF) {
          ...
          mo->s_rebase = tcc_realloc(mo->s_rebase, (mo->n_rebase + 1) *
...
static int check_symbols(TCCState *s1, struct macho *mo) {
  ...
  for (sym_index = 1; sym_index < sym_end; ++sym_index) {
    ...
    dprintf("%4d (%4d): %09lx %4d %4d %4d %3d %s\n", sym_index, elf_index,
            (long)sym->st_value, type, bind, vis, sym->st_shndx, name);
    if (bind == STB_LOCAL) {
      if (mo->ilocal == -1)
...
static void convert_symbol(TCCState *s1, struct macho *mo,
  ...
  case STT_FILE:
    ...
    break;
  default:
    tcc_error("unhandled ELF symbol type %d %s", ELFW(ST_TYPE)(sym->st_info),
              name);
  }
  if (sym->st_shndx == SHN_UNDEF)
...
static void create_symtab(TCCState *s1, struct macho *mo) {
...
#ifdef CONFIG_NEW_MACHO
  mo->chained_fixups =
      new_section(s1, "CHAINED_FIXUPS", SHT_LINKEDIT, SHF_ALLOC | SHF_WRITE);
#else
  mo->stub_helper =
      new_section(s1, "__stub_helper", SHT_PROGBITS, SHF_ALLOC | SHF_EXECINSTR);
  mo->la_symbol_ptr =
      new_section(s1, "__la_symbol_ptr", SHT_PROGBITS, SHF_ALLOC | SHF_WRITE);
  mo->helpsym =
      put_elf_sym(s1->symtab, 0, 0, ELFW(ST_INFO)(STB_LOCAL, STT_SECTION), 0,
  ...
  mo->dyld_private = put_elf_sym(s1->symtab, data_section->data_offset,
                                 PTR_SIZE, ELFW(ST_INFO)(STB_LOCAL, STT_OBJECT),
                                 0, data_section->sh_num, ".__dyld_private");
  section_ptr_add(data_section, PTR_SIZE);
  mo->dyld_stub_binder =
      put_elf_sym(s1->symtab, 0, 0, ELFW(ST_INFO)(STB_GLOBAL, STT_OBJECT), 0,
  ...
  mo->weak_binding =
      new_section(s1, "WEAK_BINDING", SHT_LINKEDIT, SHF_ALLOC | SHF_WRITE);
  mo->lazy_binding =
      new_section(s1, "LAZY_BINDING", SHT_LINKEDIT, SHF_ALLOC | SHF_WRITE);
#endif
...
const struct {
  ...
  const char *name;
    ...
    /*[sk_stubs] =*/
    {1,
     S_REGULAR | S_ATTR_PURE_INSTRUCTIONS | S_SYMBOL_STUBS |
...
static void calc_fixup_size(TCCState *s1, struct macho *mo) {
  ...
  size += (sizeof(struct dyld_chained_starts_in_image) +
           (mo->nseg - 1) * sizeof(uint32_t) + 7) &
          -8;
  for (i = (s1->output_type == TCC_OUTPUT_EXE); i < mo->nseg - 1; i++) {
    int page_count =
        (get_segment(mo, i)->vmsize + SEG_PAGE_SIZE - 1) / SEG_PAGE_SIZE;
    size += (sizeof(struct dyld_chained_starts_in_segment) +
             (page_count - 1) * sizeof(uint16_t) + 7) &
            -8;
  }
...
static void set_segment_and_offset(TCCState *s1, struct macho *mo, addr_t addr,
                                   uint8_t *ptr, int opcode, Section *sec,
                                   addr_t offset) {
  int i;
...
static void bind_rebase(TCCState *s1, struct macho *mo) {
  ...
  for (i = 0; i < mo->n_lazy_bind; i++) {
    ...
    set_segment_and_offset(
        s1, mo, mo->la_symbol_ptr->sh_addr, ptr,
  ...
  for (i = 0; i < mo->n_rebase; i++) {
    ...
    *ptr++ = REBASE_OPCODE_SET_TYPE_IMM | REBASE_TYPE_POINTER;
    set_segment_and_offset(
        s1, mo, s->sh_addr, ptr, REBASE_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB,
  ...
  for (i = 0; i < mo->n_bind; i++) {
    ...
    sym = &((ElfW(Sym) *)symtab_section->data)[sym_index];
    name = (char *)symtab_section->link->data + sym->st_name;
    binding = ELFW(ST_BIND)(sym->st_info) == STB_WEAK ? mo->weak_binding
                                                      : mo->binding;
    ...
    *ptr++ = BIND_OPCODE_SET_TYPE_IMM | BIND_TYPE_POINTER;
    set_segment_and_offset(s1, mo, s->sh_addr, ptr,
                           BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB, binding,
...
static void create_trie(struct trie_node *node, int from, int to,
                        int index_start, int n_trie, struct trie_info *trie) {
  int i;
  ...
  for (i = from; i < to; i = end) {
    ...
    if (start == end - 1 || (trie[start].name[index_start] &&
    ...
    node->child = tcc_realloc(node->child,
                              (node->n_child + 1) * sizeof(struct trie_node));
...
static int create_seq(int *offset, int *n_seq, struct trie_seq **seq,
                      struct trie_node *node, int n_trie,
  ...
  for (i = 0; i < node->n_child; i++) {
    ...
    *seq = tcc_realloc(*seq, (*n_seq + 1) * sizeof(struct trie_seq));
    ...
    p_seq->nest_offset = 0;
    *offset +=
        (i == 0 ? 1 + 1 : 0) + p_nest->index_end - p_nest->index_start + 1 + 3;
  }
  for (i = 0; i < node->n_child; i++) {
...
static void collect_sections(TCCState *s1, struct macho *mo,
  ...
  for (i = s1->nb_sections; i-- > 1;) {
    ...
    if ((flags & SHF_ALLOC) || !strncmp(s->name, ".debug_", 7)) {
      if (flags & SHF_TLS) {
        if (type == SHT_NOBITS)
          sk = sk_thread_bss;
        else if (!strcmp(s->name, "__thread_vars"))
          sk = sk_thread_vars;
        else
          sk = sk_thread_data;
      } else if (!strcmp(s->name, "__thread_vars") ||
                 !strcmp(s->name, "__thread_bss") ||
                 !strcmp(s->name, "__thread_data")) {
        if (!strcmp(s->name, "__thread_bss") || type == SHT_NOBITS)
          sk = sk_thread_bss;
        else if (!strcmp(s->name, "__thread_vars"))
          sk = sk_thread_vars;
        else
...
#ifdef CONFIG_NEW_MACHO
  chained_fixups_lc =
      add_lc(mo, LC_DYLD_CHAINED_FIXUPS, sizeof(struct linkedit_data_command));
  export_trie_lc =
      add_lc(mo, LC_DYLD_EXPORTS_TRIE, sizeof(struct linkedit_data_command));
#else
  ...
  for (sk = sk_unknown; sk < sk_last; sk++) {
    ...
    if (seg) {
      ...
      if (sk == sk_linkedit && mo->sk_to_sect[sk_thread_vars].s)
        seen_tlv = 1;
      if (seen_tlv && strcmp(seg->segname, "__DATA") == 0 && seg->vmsize < 24)
        seg->vmsize = 24;
    }
#ifdef CONFIG_NEW_MACHO
    if (sk == sk_linkedit) {
    ...
    if (skinfo[sk].seg_initial &&
      ...
      for (s = mo->sk_to_sect[sk].s; s; s = s->prev) {
        ...
        unsigned long old_do;
        int tlv_min = (mo->sk_to_sect[sk_thread_vars].s &&
                       (strcmp(s->name, "__thread_vars") == 0 ||
                        strcmp(s->name, "__thread_data") == 0 ||
                        strcmp(s->name, "__thread_bss") == 0));
        if (a && al < (a - 1))
          al = a - 1;
        s->sh_size = s->data_offset;
        /* TLV descriptor is 24 bytes; dyld uses __thread_data/__thread_bss size
         * as "total size" and rejects < 24 */
        if (tlv_min && s->sh_size < 24) {
          old_do = s->data_offset;
          if (s->sh_type != SHT_NOBITS && s->data) {
            section_realloc(s, 24);
            memset(s->data + old_do, 0, 24 - old_do);
          } else if (s->sh_type == SHT_NOBITS) {
            section_realloc(s, 24);
            memset(s->data, 0, 24);
            s->sh_type = SHT_PROGBITS;
          }
          s->data_offset = s->sh_size = 24;
        }
      }
      if (sec)
        sec->align = al;
      ...
      if (sec) {
        ...
        if ((sec->flags & SECTION_TYPE) == S_ZEROFILL ||
            (sec->flags & SECTION_TYPE) == S_THREAD_LOCAL_ZEROFILL)
          sec->offset = 0;
      }
      for (s = mo->sk_to_sect[sk].s; s; s = s->prev) {
        al = s->sh_addralign;
      ...
      if (sec) {
        ...
        if (skinfo[sk].name && strcmp(skinfo[sk].name, "__thread_vars") == 0 &&
            sec->size < 24)
          sec->size = 24;
      }
    }
...
static void macho_write(TCCState *s1, struct macho *mo, FILE *fp) {
  ...
  if (s1->output_type == TCC_OUTPUT_EXE) {
    ...
    for (i = 1; i < s1->nb_sections; i++) {
      ...
      if (sec && strcmp(sec->name, "__thread_vars") == 0) {
        has_tlv = 1;
        break;
      }
    }
...
#ifndef CONFIG_NEW_MACHO
    /* Old dyld path: we use BIND_SPECIAL_DYLIB_FLAT_LOOKUP, so flat namespace
     */
    if (mo->dyldinfo->bind_size != 0 || mo->dyldinfo->lazy_bind_size != 0)
      mo->mh.mh.flags |= MH_FORCE_FLAT;
    else
      mo->mh.mh.flags |= MH_TWOLEVEL;
    if (mo->dyldinfo->bind_size == 0 && mo->dyldinfo->lazy_bind_size == 0)
      mo->mh.mh.flags |= MH_NOUNDEFS;
#else
    /* Chained fixups path: we use lib_ordinal -2 (flat lookup), so flat
     * namespace */
    if (mo->n_bind != 0)
      mo->mh.mh.flags |= MH_FORCE_FLAT;
    else
...
static int bind_rebase_cmp(const void *_a, const void *_b, void *arg) {
  ...
  Section *sa, *sb;
  addr_t aa, ab;
  if ((unsigned)a->section >= (unsigned)s1->nb_sections ||
      (unsigned)b->section >= (unsigned)s1->nb_sections)
    return 0;
  sa = s1->sections[a->section];
  sb = s1->sections[b->section];
  if (!sa || !sb)
    return 0;
  aa = sa->sh_addr + a->rel.r_offset;
  ab = sb->sh_addr + b->rel.r_offset;
...
ST_FUNC void bind_rebase_import(TCCState *s1, struct macho *mo) {
  ...
  for (i = 0; i < mo->n_bind_rebase - 1; i++) {
    unsigned si = mo->bind_rebase[i].section;
    if (si < (unsigned)s1->nb_sections && s1->sections[si] &&
        mo->bind_rebase[i].section == mo->bind_rebase[i + 1].section &&
        mo->bind_rebase[i].rel.r_offset ==
            mo->bind_rebase[i + 1].rel.r_offset) {
  ...
  /* VM offset base: first loadable segment (__TEXT for EXE) */
  seg = get_segment(mo, s1->output_type == TCC_OUTPUT_EXE ? 1 : 0);
  image_base = seg->vmaddr;
  for (i = (s1->output_type == TCC_OUTPUT_EXE); i < mo->nseg - 1; i++) {
    image->seg_info_offset[i] =
        (data - mo->chained_fixups->data) - header->starts_offset;
    seg = get_segment(mo, i);
...
#define PTR_64_MASK 0x7FFFFFFFFFFULL
...
#endif
    /* VM offset from image base (dyld expects this, not file offset) */
    segment->segment_offset = seg->vmaddr - image_base;
    ...
    for (j = 0; j < page_count; j++) {
      ...
      for (; k < mo->n_bind_rebase; k++) {
        unsigned sec_idx = mo->bind_rebase[k].section;
        Section *s;
        if (sec_idx >= (unsigned)s1->nb_sections)
          tcc_error("bind_rebase: invalid section index %u", sec_idx);
        s = s1->sections[sec_idx];
        if (!s || !s->data)
          tcc_error("bind_rebase: section %u has no data", sec_idx);
        {
          addr_t r_offset, addr;
          r_offset = mo->bind_rebase[k].rel.r_offset;
          if (r_offset >= s->data_offset)
            tcc_error("bind_rebase: r_offset %llu >= section size %u",
                      (unsigned long long)r_offset, (unsigned)s->data_offset);
          addr = s->sh_addr + r_offset;

          if ((addr & 3) ||
              (addr & (SEG_PAGE_SIZE - 1)) > SEG_PAGE_SIZE - PTR_SIZE)
            tcc_error("Illegal rel_offset %s %lld", s->name,
                      (long long)r_offset);
          if (addr >= end)
            break;
          if (addr >= start) {
            ...
            if (mo->bind_rebase[k].bind) {
              ...
              rebase =
                  (struct dyld_chained_ptr_64_rebase *)(s->data + r_offset);
              last = rebase;
              last_o = cur_o;
...
ST_FUNC int macho_output_file(TCCState *s1, const char *filename) {
  ...
  if (!ret) {
    ...
    if (s1->output_type == TCC_OUTPUT_EXE) {
      struct segment_command_64 *seg1 = get_segment(&mo, 1);
      /* LC_MAIN entryoff is file offset of entry point, not vm offset */
      mo.ep->entryoff =
          seg1->fileoff + (get_sym_addr(s1, "main", 1, 1) - seg1->vmaddr);
    }
    if (s1->nb_errors)
      goto do_ret;
...
/* Mach-O relocatable object loader: merge sections/symbols/relocs into TCC
 * state */
...
#define MACHO_R_SYMBOLNUM(r) ((r).r_info & 0x00ffffffu)
#define MACHO_R_PCREL(r) (((r).r_info >> 24) & 0x1u)
#define MACHO_R_LENGTH(r) (((r).r_info >> 25) & 0x3u)
#define MACHO_R_EXTERN(r) (((r).r_info >> 27) & 0x1u)
#define MACHO_R_TYPE(r) (((r).r_info >> 28) & 0xfu)
...
static Section *macho_find_section(TCCState *s1, const char *name) {
  int i;
  for (i = 1; i < s1->nb_sections; i++)
    if (!strcmp(s1->sections[i]->name, name))
      return s1->sections[i];
  return NULL;
}

static const char *macho_sect_to_tcc_name(const char *segname,
                                          const char *sectname) {
  if (!strcmp(sectname, "__text"))
    return ".text";
  if (!strcmp(sectname, "__data"))
    return ".data";
...
ST_FUNC int macho_load_object_file(TCCState *s1, int fd,
  ...
  struct load_command *lc;
  void *cmds = NULL;
  unsigned long machofs = file_offset;
  uint32_t i, j, ncmds, symoff = 0, nsyms = 0, stroff = 0, strsize = 0;
  struct nlist_64 *nlist = NULL;
  char *strtab = NULL;
  Section **sect_index_to_section = NULL;
  int nsects = 0;
  int *old_to_new_syms = NULL;
  int *section_sym_index = NULL;
  int ret = -1;
  size_t *section_offset = NULL;
  uint64_t *section_addr = NULL; /* original addr of each section in the .o */
  Section *tls_data = NULL;
  Section *tls_bss = NULL;
  unsigned long tls_data_size = 0;
  unsigned long tls_bss_base = 0;

  lseek(fd, file_offset, SEEK_SET);
  if (full_read(fd, buf, 8) != 8) {
    tcc_error_noabort("macho: short read (header)");
    return -1;
  }
  if (*(uint32_t *)buf == FAT_MAGIC || *(uint32_t *)buf == FAT_CIGAM) {
    struct fat_arch *fa = NULL;
    uint32_t nfat = ((uint32_t *)buf)[1];
    int swap = (*(uint32_t *)buf == FAT_CIGAM);
    if (swap)
      nfat = macho_swap32(nfat);
    fa = tcc_malloc(sizeof(*fa) * nfat);
    if (!fa)
      return -1;
    lseek(fd, file_offset + sizeof(struct fat_header), SEEK_SET);
    if (full_read(fd, fa, sizeof(*fa) * nfat) !=
        (ssize_t)(sizeof(*fa) * nfat)) {
      tcc_free(fa);
      return -1;
    }
    for (i = 0; i < nfat; i++) {
      uint32_t cputype = swap ? macho_swap32(fa[i].cputype) : fa[i].cputype;
      uint32_t cpusubtype =
          swap ? macho_swap32(fa[i].cpusubtype) : fa[i].cpusubtype;
#ifdef TCC_TARGET_X86_64
      if (cputype == CPU_TYPE_X86_64 &&
          (cpusubtype & 0xff) == CPU_SUBTYPE_X86_ALL)
#elif defined TCC_TARGET_ARM64
      ...
      {
        machofs =
            file_offset + (swap ? macho_swap32(fa[i].offset) : fa[i].offset);
        break;
      }
    }
    if (i == nfat) {
      tcc_error_noabort("macho: no matching arch in fat binary");
      tcc_free(fa);
      return -1;
    }
    tcc_free(fa);
  ...
  lseek(fd, machofs, SEEK_SET);
  if (full_read(fd, buf, sizeof(struct mach_header_64)) !=
      sizeof(struct mach_header_64)) {
    tcc_error_noabort("macho: short read (mach_header_64)");
    return -1;
  }
  memcpy(&mh, buf, sizeof(mh));
  if (mh.mh.magic != MH_MAGIC_64) {
    tcc_error_noabort("macho: bad magic 0x%x", (unsigned)mh.mh.magic);
    return -1;
  }
  if (mh.mh.filetype != MH_OBJECT && mh.mh.filetype != MH_DYLIB) {
    tcc_error_noabort("macho: unsupported filetype %u",
                      (unsigned)mh.mh.filetype);
    return -1;
  }
  ncmds = mh.mh.ncmds;
  cmds =
      load_data(fd, machofs + sizeof(struct mach_header_64), mh.mh.sizeofcmds);
  if (!cmds) {
    tcc_error_noabort("macho: failed to load load commands");
    return -1;
  }

  /* First pass: count sections and collect LC_SYMTAB */
  for (i = 0, lc = cmds; i < ncmds; i++) {
  ...
  if (!nsyms || !strsize) {
    tcc_error_noabort("macho: no symbol table (nsyms=%u strsize=%u)",
                      (unsigned)nsyms, (unsigned)strsize);
    tcc_free(cmds);
    return -1;
  }
  sect_index_to_section = tcc_mallocz(sizeof(Section *) * (nsects + 2));
  section_offset = tcc_mallocz(sizeof(size_t) * (nsects + 2));
  section_addr = tcc_mallocz(sizeof(uint64_t) * (nsects + 2));
  section_sym_index = tcc_mallocz(sizeof(int) * (nsects + 2));

  /* Second pass: create/find sections and load data */
  nsects = 0;
  for (i = 0, lc = cmds; i < ncmds; i++) {
    if (lc->cmd != LC_SEGMENT_64) {
      lc = (struct load_command *)((char *)lc + lc->cmdsize);
    ...
    }
    {
      ...
      for (j = 0; j < seg->nsects; j++, sec++) {
        uint32_t sect_type;
        nsects++;
        sect_type = sec->flags & SECTION_TYPE;
        section_addr[nsects] = sec->addr; /* save .o-relative base addr */
        tcc_name = macho_sect_to_tcc_name(seg->segname, sec->sectname);
        tcc_sec = macho_find_section(s1, tcc_name);
        if (!tcc_sec) {
          int is_tls = (sect_type == S_THREAD_LOCAL_REGULAR ||
                        sect_type == S_THREAD_LOCAL_ZEROFILL ||
                        sect_type == S_THREAD_LOCAL_VARIABLES ||
                        sect_type == S_THREAD_LOCAL_VARIABLE_POINTERS ||
                        sect_type == S_THREAD_LOCAL_INIT_FUNCTION_POINTERS);
          int sh_type =
              (sect_type == S_THREAD_LOCAL_ZEROFILL || sect_type == S_ZEROFILL)
                  ? SHT_NOBITS
                  : SHT_PROGBITS;
          int sh_flags = SHF_ALLOC;
          if (!strcmp(seg->segname, "__DATA") ||
              !strcmp(seg->segname, "__DATA_DIRTY"))
            sh_flags |= SHF_WRITE;
          if (!strcmp(seg->segname, "__TEXT") &&
              !strcmp(sec->sectname, "__text"))
            sh_flags |= SHF_EXECINSTR;
          if (is_tls)
            sh_flags |= SHF_TLS;
          tcc_sec = new_section(s1, tcc_name, sh_type, sh_flags);
        }
        sect_index_to_section[nsects] = tcc_sec;
        align = sec->align ? (1u << sec->align) : 1;
        if (align < 1)
          align = 1;
        section_offset[nsects] = section_add(tcc_sec, sec->size, align);
        if (!(sect_type == S_ZEROFILL ||
              sect_type == S_THREAD_LOCAL_ZEROFILL) &&
            sec->size) {
          lseek(fd, machofs + sec->offset, SEEK_SET);
          full_read(fd, tcc_sec->data + section_offset[nsects], sec->size);
          if (strstr(sec->sectname, "__text") && sec->size > 0x620) {
            uint32_t raw_insn =
                *(uint32_t *)(tcc_sec->data + section_offset[nsects] + 0x620);
            printf("Loaded __text at off=%lx, size=0x%x, raw insn at 0x620 = "
                   "0x%08x\n",
                   (long)section_offset[nsects], (unsigned)sec->size, raw_insn);
          }
        }
      }
    }
  ...
  nlist = load_data(fd, machofs + symoff, nsyms * sizeof(struct nlist_64));
  strtab = load_data(fd, machofs + stroff, strsize);
  if (!nlist || !strtab) {
    tcc_error_noabort("macho: failed to load symtab/strtab");
    tcc_free(nlist);
    tcc_free(strtab);
    tcc_free(sect_index_to_section);
    tcc_free(section_offset);
    tcc_free(section_addr);
    tcc_free(section_sym_index);
    tcc_free(cmds);
    return -1;
  }
  old_to_new_syms = tcc_mallocz(sizeof(int) * (nsyms + 1));

  /* Load symbols (index 0 is often empty) */
  for (i = 1; i < nsyms; i++) {
    struct nlist_64 *n = &nlist[i];
    const char *name;
    int info, other, shndx;
    addr_t value;
    unsigned long size = 0;
    if (n->n_strx >= strsize)
      continue;
    name = strtab + n->n_strx;
    if (!name[0])
      continue;
    value = n->n_value;
    if (n->n_type & N_EXT) {
      info = (n->n_desc & (N_WEAK_REF | N_WEAK_DEF))
                 ? ELFW(ST_INFO)(STB_WEAK, STT_NOTYPE)
                 : ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE);
      if ((n->n_type & N_TYPE) == N_SECT)
        info = ELFW(ST_INFO)(ELFW(ST_BIND)(info), STT_FUNC);
    } else {
      info = ELFW(ST_INFO)(STB_LOCAL, STT_NOTYPE);
      if ((n->n_type & N_TYPE) == N_SECT)
        info = ELFW(ST_INFO)(STB_LOCAL, STT_OBJECT);
    }
    other = 0;
    if (n->n_sect == 0)
      shndx = SHN_UNDEF;
    else if (n->n_sect <= nsects && sect_index_to_section[n->n_sect]) {
      shndx = sect_index_to_section[n->n_sect]->sh_num;
      /* n_value is absolute in the .o address space; make it
         section-relative, then offset into the TCC section. */
      value = (value - section_addr[n->n_sect]) + section_offset[n->n_sect];
    } else
      shndx = SHN_ABS;
    old_to_new_syms[i] =
        set_elf_sym(symtab_section, value, size, info, other, shndx, name);
  }

  tls_data = macho_find_section(s1, "__thread_data");
  tls_bss = macho_find_section(s1, "__thread_bss");
  if (tls_data)
    tls_data_size = tls_data->data_offset;
  tls_bss_base = tls_data_size;
  if (tls_bss && tls_bss->sh_addralign > 1)
    tls_bss_base = (tls_bss_base + tls_bss->sh_addralign - 1) &
                   ~(tls_bss->sh_addralign - 1);

  /* Third pass: relocations (same section order as second pass) */
  {
    int sect_idx = 0;
    ...
    for (i = 0, lc = cmds; i < ncmds; i++) {
      ...
      }
      {
        ...
        for (j = 0; j < seg->nsects; j++, sec++) {
          sect_idx++;
          if (!sec->nreloc || sec->reloff == 0)
            continue;
          {
            Section *tcc_sec = sect_index_to_section[sect_idx];
            size_t base_off = section_offset[sect_idx];
            struct macho_relocation_info *rel;
            uint32_t k;
            if (!tcc_sec)
              continue;
            rel = load_data(fd, machofs + sec->reloff,
                            sec->nreloc * sizeof(struct macho_relocation_info));
            if (!rel)
              continue;
            for (k = 0; k < sec->nreloc; k++) {
              int32_t addr = rel[k].r_address;
              uint32_t symnum;
              uint32_t r_type;
              int elf_type;
              addr_t addend;
              ...
              if (addr & R_SCATTERED)
                continue;
              symnum = MACHO_R_SYMBOLNUM(rel[k]);
              r_type = MACHO_R_TYPE(rel[k]);
              int r_length = MACHO_R_LENGTH(rel[k]);
              int r_pcrel = MACHO_R_PCREL(rel[k]);
              Section *sym_sec;
              int tcc_sym = 0;

              if (r_type == ARM64_RELOC_ADDEND) {
                pending_addend = (int32_t)(symnum << 8) >> 8;
                have_addend = 1;
                continue;
              }

              elf_type = 0;
              if (MACHO_R_EXTERN(rel[k])) {
                if (symnum < (uint32_t)nsyms) {
                  int n_strx = nlist[symnum].n_strx;
                  if (n_strx < (int)strsize)
                    rel_name = strtab + n_strx;
                }
                tcc_sym =
                    (symnum < (uint32_t)nsyms) ? old_to_new_syms[symnum] : 0;
                if (tcc_sym == 0)
                  continue;
              } else {
                if (symnum == 0 || symnum > (uint32_t)nsects)
                  continue;
                sym_sec = sect_index_to_section[symnum];
                if (!sym_sec)
                  continue;
                if (!section_sym_index[symnum]) {
                  section_sym_index[symnum] =
                      set_elf_sym(symtab_section, 0, 0,
                                  ELFW(ST_INFO)(STB_LOCAL, STT_SECTION), 0,
                                  sym_sec->sh_num, "");
                }
                tcc_sym = section_sym_index[symnum];
                if (tcc_sym == 0)
                  continue;
              }
              int implicit_addend_valid = 1;
              addend = 0;
              if (have_addend && (r_type == ARM64_RELOC_BRANCH26 ||
                                  ...
                                  r_type == ARM64_RELOC_TLVP_LOAD_PAGE21 ||
                                  r_type == ARM64_RELOC_TLVP_LOAD_PAGEOFF12)) {
                addend = pending_addend;
                have_addend = 0;
                implicit_addend_valid = 0;
              } else if (have_addend) {
                have_addend = 0;
              }
#ifdef TCC_TARGET_ARM64
              if (r_type == ARM64_RELOC_UNSIGNED && rel_name &&
                  strcmp(tcc_sec->name, "__thread_vars") == 0) {
                size_t name_len = strlen(rel_name);
                if (name_len > 9 &&
                    strcmp(rel_name + name_len - 9, "$tlv$init") == 0) {
                  ElfW(Sym) *sym =
                      &((ElfW(Sym) *)symtab_section->data)[tcc_sym];
                  addr_t tls_off = sym->st_value;
                  if (tls_bss && sym->st_shndx == tls_bss->sh_num)
                    tls_off += tls_bss_base;
                  if (base_off + addr + 8 <= tcc_sec->data_offset)
                    write64le(tcc_sec->data + base_off + addr,
                              tls_off + addend);
                  continue;
                }
              }
#endif
#ifdef TCC_TARGET_X86_64
              if (r_type == X86_64_RELOC_SUBTRACTOR)
                continue;
#elif defined TCC_TARGET_ARM64
...
#endif
#ifdef TCC_TARGET_X86_64
              if (r_type == X86_64_RELOC_UNSIGNED && r_length == 3)
                elf_type = R_X86_64_64;
              else if (r_type == X86_64_RELOC_SIGNED && r_length == 2 &&
                       r_pcrel)
                elf_type = R_X86_64_PC32, addend = -4;
              else if (r_type == X86_64_RELOC_BRANCH)
                elf_type = R_X86_64_PC32, addend = -4;
              else if (r_type == X86_64_RELOC_GOT_LOAD ||
                       r_type == X86_64_RELOC_GOT)
                elf_type = R_X86_64_GOTPCREL, addend = -4;
#elif defined TCC_TARGET_ARM64
              if (r_type == ARM64_RELOC_UNSIGNED && r_length == 3)
                elf_type = R_AARCH64_ABS64;
              else if (r_type == ARM64_RELOC_BRANCH26) {
                uint32_t insn = *(uint32_t *)(tcc_sec->data + base_off + addr);
                if (insn & 0x80000000)
                  elf_type = R_AARCH64_CALL26;
                else
                ...
                if ((insn & 0x3f000000) == 0x39000000) {
                  ...
                  int shift = (insn >> 22) & 3;
                  if (implicit_addend_valid)
                    addend = ((insn >> 10) & 0xfff) << (shift == 1 ? 12 : 0);
                  elf_type = R_AARCH64_ADD_ABS_LO12_NC;
                }
              } else if (r_type == ARM64_RELOC_GOT_LOAD_PAGE21 ||
                         r_type == ARM64_RELOC_TLVP_LOAD_PAGE21)
                elf_type = R_AARCH64_ADR_GOT_PAGE, addend = 0;
              else if (r_type == ARM64_RELOC_GOT_LOAD_PAGEOFF12 ||
                       r_type == ARM64_RELOC_TLVP_LOAD_PAGEOFF12)
                elf_type = R_AARCH64_LD64_GOT_LO12_NC, addend = 0;
#endif
              if (elf_type != 0)
                put_elf_reloca(symtab_section, tcc_sec, base_off + addr,
                               elf_type, tcc_sym, addend);
            }
            tcc_free(rel);
          }
        }
...
#pragma pop_macro("free")
  if (path.size)
    ...
    tcc_add_library_path(
        s, "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/lib"
           ":"
...
ST_FUNC int macho_load_tbd(TCCState *s1, int fd, const char *filename,
                           int lev) {
  char *soname, *data, *pos;
  int ret = -1;
...
again:
  ...
  if (fh.magic == FAT_MAGIC || fh.magic == FAT_CIGAM) {
    ...
    tcc_warning("%s: Mach-O fat 64bit files of type 0x%x not handled", filename,
                fh.magic);
    return -1;
  }
  ...
  for (i = iextdef; i < iextdef + nextdef; i++) {
    ...
    dprintf("%5d: %3d %3d 0x%04x 0x%016lx %s\n", i, sym->n_type, sym->n_sect,
            sym->n_desc, (long)sym->n_value, strtab + sym->n_strx);
    set_elf_sym(s1->dynsymtab_section, 0, 0,
