...
static void fill_bsd_note(Section *s, int type, const char *value,
                          uint32_t data) {
  ...
  while (offset + sizeof(ElfW(Nhdr)) < s->data_offset) {
    ...
    if (note->n_type == type)
      return;
    offset +=
        (sizeof(ElfW(Nhdr)) + note->n_namesz + note->n_descsz + align - 1) &
        -align;
  }
...
static unsigned long long get_arch_number(const uint8_t *b, int n,
                                          int little_endian) {
...
static int tcc_load_alacarte(TCCState *s1, int fd, int size, int entrysize,
  ...
  if (ranlib_format) {
    ...
    if ((unsigned long long)nsyms * entrysize >
        (unsigned long long)(size - entrysize))
      goto invalid;
    ...
    if (!ranlib_format) {
      ...
      for (i = 0; i < nsyms; i++) {
        ...
        unsigned long long strx =
            get_arch_number(entry, pair_size, little_endian);
        const char *name;
        Section *s = symtab_section;
...
static int archive_symtab_entrysize(const char *name, int *little_endian,
                                    int *ranlib_format) {
  char trimmed[32];
