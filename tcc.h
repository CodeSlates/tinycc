...
struct SymAttr {
  unsigned short aligned : 5, /* alignment as log2+1 (0 == unspecified) */
      ...
      dllimport : 1, addrtaken : 1, nodebug : 1, xxxx : 2; /* not used */
};
...
struct TCCState {
  ...
  unsigned char nostdlib_paths; /* if true, the default paths are not searched
                                   for libraries */
...
#define IS_ASM_SYM(sym)                                                        \
  (((sym)->type.t &                                                            \
    ((VT_BTYPE | VT_STRUCT_MASK) & ~(1 << VT_STRUCT_SHIFT))) == VT_ASM)
...
#define dwarf_read_2(ln, end) ((ln) + 1 < (end) ? read16le(((ln) += 2) - 2) : 0)
#define dwarf_read_4(ln, end) ((ln) + 3 < (end) ? read32le(((ln) += 4) - 4) : 0)
#define dwarf_read_8(ln, end) ((ln) + 7 < (end) ? read64le(((ln) += 8) - 8) : 0)
